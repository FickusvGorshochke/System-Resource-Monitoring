...
static void *producer(void *arg)
{
    ...
    while (g_running) {
        ...
        g_buf[g_in] = item; g_in = (g_in + 1) % BUF_N;
        pthread_mutex_unlock(&g_mtx);
        sem_post(&g_full);
        trace_logi(960, (unsigned)item, 0);
        item++; g_produced++;
...
static void *consumer(void *arg)
{
    ...
    while (g_running) {
        sem_wait(&g_full);
        pthread_mutex_lock(&g_mtx);
        (void)g_buf[g_out]; g_out = (g_out + 1) % BUF_N;
        pthread_mutex_unlock(&g_mtx);
        sem_post(&g_empty);
        if (g_consumed % LOG_EVERY == 0)
            trace_logi(961, (unsigned)g_consumed, 0);
        g_consumed++;
    }
    return NULL;
}
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "case_prod_cons_instr: pid=%d буфер=%d период=%uмс\n",
            (int)getpid(), BUF_N, g_prod_ms);
    trace_logf(962, "prod_cons start buf=%d", BUF_N);
    ...
    if (seconds > 0) sleep(seconds); else while (g_running) sleep(1);
    g_running = 0;
    sem_post(&g_full);
    pthread_join(tp, NULL); pthread_join(tc, NULL);

    fprintf(stderr, "case_prod_cons_instr: произведено=%lu потреблено=%lu\n",
