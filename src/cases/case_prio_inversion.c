...
static void busy_ms(uint32_t ms)
{
    ...
    while (mono_ns() < end)
        for (int i = 0; i < 1000; i++) a = a * 1103515245ULL + 12345ULL;
    g_dummy = a;
}
...
static void report_prio(const char *who)
{
    int policy; struct sched_param sp;
    pthread_getschedparam(pthread_self(), &policy, &sp);
    int d[3] = {1,1,1};
    ThreadCtl(_NTO_TCTL_RUNMASK_GET_AND_SET, d);
    fprintf(stderr, "  [%s] prio=%d policy=%d runmask=0x%x\n",
            who, sp.sched_priority, policy, d[1]);
}
...
static pthread_t spawn(int prio, void *(*fn)(void *))
{
    ...
    if (pthread_create(&t, &at, fn, NULL) != 0) {
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "case_prio_inversion: pid=%d наследование=%s "
            "(L=%d M=%d H=%d, крит.секция=%uмс, загрузка M=%uмс)\n",
            (int)getpid(), use_pi ? "ВКЛ" : "выкл",
            L, M, H, g_low_work_ms, g_mid_burn_ms);
    ...
    fprintf(stderr, "case_prio_inversion: завершение\n");
    return 0;
}
